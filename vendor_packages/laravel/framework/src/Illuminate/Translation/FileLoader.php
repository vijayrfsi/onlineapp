...
class FileLoader implements Loader
{
    ...
    public function load($locale, $group, $namespace = null)
    {
        ...
        if (is_null($namespace) || $namespace === '*') {
            if(count($this->jsonPaths) && ($filePath = $this->matchPath($group))){
                return $this->loadPath($filePath, $locale, $group);
            } else 
    ...
    public function matchPath($group){
        $selectedPath = 0;
        foreach($this->jsonPaths as $path){
            if(stristr($path, "/modules/$group/resources/")){
                $selectedPath = $path;
                break;
            }
        }
